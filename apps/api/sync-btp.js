/**
 * Fast Baselinker Sync Script - BTP
 * Pobiera WSZYSTKIE produkty z magazynu BTP (ID: 22953)
 * 
 * Uruchom: node sync-btp.js
 */
...
const INVENTORY_ID = '22953'; // BTP
const ERROR_LOG_FILE = path.join(__dirname, '..', 'SYNC_ERRORS_REPORT.md');

// Funkcja do logowania błędów do pliku MD
function logError(sku, productId, errorMessage, productName = '') {
  const timestamp = new Date().toISOString();
  const logEntry = `
### Błąd synchronizacji BTP - ${timestamp}
...
async function main() {
  console.log('🚀 Fast Baselinker Sync - BTP (WSZYSTKIE produkty)');
  console.log('====================================================\n');
  ...
  console.log('📂 Synchronizacja kategorii z magazynu BTP...');
  ...
  console.log(`   Znaleziono ${blCategories.length} kategorii w Baselinker BTP`);
  ...
  // Create categories with BTP prefix
  for (const blCat of blCategories) {
    const blCatId = 'btp-' + blCat.category_id.toString();
    const displayName = '[BTP] ' + getDisplayName(blCat.name);
    const slug = 'btp-' + slugify(getDisplayName(blCat.name)) + '-' + blCat.category_id;
  ...
  // Get existing products from BTP
  ...
  // Filter new products
  const newProducts = allProducts.filter(p => !existingIds.has('btp-' + p.id.toString()));
  console.log(`📦 Nowych produktów do pobrania: ${newProducts.length}\n`);
  
  if (newProducts.length === 0) {
    console.log('✅ Wszystkie testowe produkty z BTP są już zsynchronizowane!');
    ...
    for (const [productId, blProduct] of Object.entries(productsObj)) {
      try {
        if (!blProduct || typeof blProduct !== 'object') {
          const errorMsg = 'Pusty produkt';
          console.error(`   ✗ ${errorMsg} ID: ${productId}`);
          logError('N/A', productId, errorMsg, 'N/A');
          errors++;
          errorDetails.push({ sku: 'N/A', productId, error: errorMsg });
          continue;
        }
        
        const baselinkerProductId = 'btp-' + productId;
        const name = getProductName(blProduct) || `Product ${productId}`;
        const baseSku = blProduct.sku || productId.toString();
        const sku = 'btp-' + baseSku; // UNIQUE per warehouse
        const slug = 'btp-' + slugify(name) + '-' + productId;
        ...
        let tags = ['btp']; // Dodaj tag "btp" do wszystkich produktów
        ...
        // Znajdź kategorię po baselinkerCategoryId z prefiksem btp
        let categoryId = null;
        if (blProduct.category_id) {
          const category = await prisma.category.findUnique({
            where: { baselinkerCategoryId: 'btp-' + blProduct.category_id.toString() }
          });
          if (category) {
        ...
        errors++;
        const sku = 'BTP-' + (blProduct?.sku || productId);
        const name = getProductName(blProduct) || 'N/A';
        console.error(`   ✗ Błąd dla produktu ${productId} (SKU: ${sku}): ${err.message}`);
        
        // Loguj błąd do pliku
        logError(sku, productId, err.message, name);
        errorDetails.push({ sku, productId, error: err.message, name });
      }
    }
  }
  ...
  for (const stockEntry of stockProducts) {
    const blId = 'btp-' + stockEntry.product_id.toString();
  ...
  console.log(`   ✓ Utworzono: ${processed} produktów z magazynu BTP`);
  console.log(`   ✗ Błędów: ${errors}`);
  console.log(`   📊 Stany: ${stockUpdated}`);
  console.log(`   📦 Razem produktów BTP w bazie: ${processed + existing.length}`);
  
  if (errors > 0) {
    console.log(`\n📝 Szczegóły błędów zapisane w: ${ERROR_LOG_FILE}`);
    console.log('\nBłędy:');
    errorDetails.forEach(detail => {
      console.log(`  - SKU: ${detail.sku} | ID: ${detail.productId} | ${detail.error}`);
    });
  }
