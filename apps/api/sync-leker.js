/**
 * Fast Baselinker Sync Script - Leker
 * Pobiera WSZYSTKIE produkty z magazynu leker (ID: 22952)
 * 
 * Uruchom: node sync-leker.js
 */
...
async function main() {
  console.log('🚀 Fast Baselinker Sync - Leker (WSZYSTKIE produkty)');
  console.log('====================================================\n');
  ...
  console.log('📂 Synchronizacja kategorii z magazynu Leker...');
  ...
  console.log(`   Znaleziono ${blCategories.length} kategorii w Baselinker Leker`);
  ...
  // Create categories with Leker prefix
  for (const blCat of blCategories) {
    const blCatId = 'leker-' + blCat.category_id.toString();
    const displayName = '[Leker] ' + getDisplayName(blCat.name);
    const slug = 'leker-' + slugify(getDisplayName(blCat.name)) + '-' + blCat.category_id;
  ...
  // Get existing products from Leker
  ...
  // Filter new products
  const newProducts = allProducts.filter(p => !existingIds.has('leker-' + p.id.toString()));
  console.log(`📦 Nowych produktów do pobrania: ${newProducts.length}\n`);
  
  if (newProducts.length === 0) {
    console.log('✅ Wszystkie produkty z Leker są już zsynchronizowane!');
  ...
  for (let i = 0; i < newProducts.length; i += BATCH_SIZE) {
    ...
    for (const [productId, blProduct] of Object.entries(productsObj)) {
      try {
        ...
        const baselinkerProductId = 'leker-' + productId;
        const name = getProductName(blProduct) || `Product ${productId}`;
        const baseSku = blProduct.sku || productId.toString();
        const sku = 'leker-' + baseSku; // UNIQUE per warehouse
        const slug = 'leker-' + slugify(name) + '-' + productId;
        ...
        let tags = ['leker']; // Dodaj tag "leker" do wszystkich produktów
        ...
        // Znajdź kategorię po baselinkerCategoryId z prefiksem leker
        let categoryId = null;
        if (blProduct.category_id) {
          const category = await prisma.category.findUnique({
            where: { baselinkerCategoryId: 'leker-' + blProduct.category_id.toString() }
          });
          if (category) {
  ...
  for (const stockEntry of stockProducts) {
    const blId = 'leker-' + stockEntry.product_id.toString();
  ...
  console.log(`   ✓ Utworzono: ${processed} produktów z magazynu Leker`);
  console.log(`   ✗ Błędów: ${errors}`);
  console.log(`   📊 Stany: ${stockUpdated}`);
  console.log(`   📦 Razem produktów Leker w bazie: ${processed + existing.length}`);
