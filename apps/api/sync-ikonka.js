/**
 * Fast Baselinker Sync Script - Ikonka
 * Pobiera WSZYSTKIE produkty z magazynu Ikonka (ID: 22951)
 * 
 * Uruchom: node sync-ikonka.js
 */
...
async function main() {
  console.log('🚀 Fast Baselinker Sync - Ikonka (WSZYSTKIE produkty)');
  console.log('======================================================\n');
  ...
  console.log('📂 Synchronizacja kategorii z magazynu Ikonka...');
  ...
  console.log(`   Znaleziono ${blCategories.length} kategorii w Baselinker Ikonka`);
  ...
  // Create categories without prefix for Ikonka
  for (const blCat of blCategories) {
    const blCatId = blCat.category_id.toString(); // NO PREFIX for Ikonka
    const displayName = '[Ikonka] ' + getDisplayName(blCat.name);
    const slug = slugify(getDisplayName(blCat.name)) + '-' + blCat.category_id;
  ...
  // Get existing products from Ikonka (no prefix - exclude other warehouses)
  ...
  // Filter new products
  const newProducts = allProducts.filter(p => !existingIds.has(p.id.toString())); // NO PREFIX
  console.log(`📦 Nowych produktów do pobrania: ${newProducts.length}\n`);
  
  if (newProducts.length === 0) {
    console.log('✅ Wszystkie produkty z Ikonka są już zsynchronizowane!');
  ...
  for (let i = 0; i < newProducts.length; i += BATCH_SIZE) {
    ...
    for (const [productId, blProduct] of Object.entries(productsObj)) {
      try {
        ...
        const baselinkerProductId = productId; // NO PREFIX for Ikonka
        const name = getProductName(blProduct) || `Product ${productId}`;
        const baseSku = blProduct.sku || productId.toString();
        const sku = 'ikonka-' + baseSku; // UNIQUE per warehouse
        ...
        let tags = ['ikonka']; // Dodaj tag "ikonka" do wszystkich produktów
        ...
        // Znajdź kategorię po baselinkerCategoryId bez prefiksu
        let categoryId = null;
        if (blProduct.category_id) {
          const category = await prisma.category.findUnique({
            where: { baselinkerCategoryId: blProduct.category_id.toString() } // NO PREFIX
          });
          if (category) {
  ...
  for (const stockEntry of stockProducts) {
    const blId = stockEntry.product_id.toString(); // NO PREFIX for Ikonka
  ...
  console.log(`   ✓ Utworzono: ${processed} produktów z magazynu Ikonka`);
  console.log(`   ✗ Błędów: ${errors}`);
  console.log(`   📊 Stany: ${stockUpdated}`);
  console.log(`   📦 Razem produktów Ikonka w bazie: ${processed + existing.length}`);
