...
export default function ShipPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      if (varRes.ok) {
        ...
        // Only show variants with available stock
  ...
  const addItem = (variant: Variant, location: VariantLocation) => {
    // Check if already added from same location
    if (items.some(i => i.variantId === variant.id && i.locationId === location.locationId)) {
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    for (const item of items) {
      if (item.quantity > item.maxAvailable) {
        setError(`Niewystarczająca ilość dla ${item.variant?.product.name}`);
        return;
      }
    }
    ...
    try {
      ...
      // Process each item
      for (const item of items) {
        const res = await api.post('/api/inventory/ship', {
        ...
        if (!res.ok) {
          ...
          throw new Error(data.message || 'Błąd wydania towaru');
        }
      }
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Towar wydany!</h2>
          <p className="text-gray-400">Wydano {items.length} pozycji ({totalItems} szt.)</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Wydanie towaru (WZ)</h1>
          <p className="text-gray-400">Wydaj towar z magazynu</p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="np. WZ/2024/001 lub numer zamówienia"
                      ...
                      <div className="pl-13 space-y-1">
                        {variant.locations.filter(l => l.available > 0).map(loc => (
                          <button
                            ...
                            disabled={items.some(i => i.variantId === variant.id && i.locationId === loc.locationId)}
            ...
            {items.length === 0 ? (
              ...
              <div className="space-y-3">
                {items.map((item, index) => {
                  const location = item.variant?.locations.find(l => l.locationId === item.locationId);
                  return (
                    <div
                      key={item.id}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 flex items-center gap-1 mt-1">
                          ...
                          {location?.locationName} • Dostępne: {item.maxAvailable}
                        </p>
                      </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-6 sticky top-6">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-red-500 text-white rounded-lg hover:bg-red-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium"
            >
              {saving ? (
                <>
