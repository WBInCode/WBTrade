...
function UnsubscribeContent() {
  ...
  useEffect(() => {
    if (!token) {
      setStatus('error');
      setMessage('Brak tokenu do wypisania.');
      return;
    }
        ...
        <div className="max-w-lg mx-auto text-center">
          {status === 'loading' && (
            <div className="bg-white dark:bg-secondary-800 rounded-xl shadow-sm border border-gray-100 dark:border-secondary-700 p-8">
              ...
              <p className="text-gray-600 dark:text-gray-400">Przetwarzam...</p>
            </div>
          )}
          ...
          {status === 'success' && (
            <div className="bg-white dark:bg-secondary-800 rounded-xl shadow-sm border border-gray-100 dark:border-secondary-700 p-8">
              ...
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Wypisano</h1>
              <p className="text-gray-600 dark:text-gray-400 mb-6">{message}</p>
              <p className="text-gray-500 dark:text-gray-500 text-sm mb-6">
                Nie będziesz już otrzymywać od nas wiadomości. Możesz zapisać się ponownie w dowolnym momencie.
              </p>
              <Link
                href="/"
