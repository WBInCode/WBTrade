    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="container-custom py-4 sm:py-6 lg:py-8 flex-1">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6 lg:mb-8">
          <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900">Koszyk</h1>
          {!isEmpty && (
            <button
              onClick={() => clearCart()}
          ...
          <div className="grid lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            ...
            <div className="lg:col-span-2 space-y-3 sm:space-y-4">
              {cart.items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`bg-white rounded-lg p-3 sm:p-4 transition-opacity ${isUpdating ? 'opacity-50' : ''}`}
                  >
                    {/* Mobile Layout */}
                    <div className="flex flex-col sm:hidden gap-2.5">
                      <div className="flex gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <Link href={`/products/${item.variant.product.id}`}>
                            <h3 className="font-medium text-xs leading-tight text-gray-900 hover:text-orange-500 transition-colors line-clamp-2">
                              {item.variant.product.name}
                            </h3>
                          </Link>
                          <p className="text-[11px] text-gray-500 mt-0.5">{item.variant.name}</p>
                          <p className={`text-[11px] mt-0.5 ${availableStock > 0 ? 'text-green-600' : 'text-red-500'}`}>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div>
                          ...
                          <p className="text-[11px] text-gray-500">{item.variant.price.toFixed(2)} zł / szt.</p>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="w-7 h-7 rounded-full border border-gray-300 flex items-center justify-center text-gray-600 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                          ...
                          <input
                            ...
                            disabled={isUpdating}
                            className="w-10 text-center text-sm font-medium border border-gray-300 rounded-lg py-1 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent disabled:opacity-50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                          />
                          <button
                            onClick={() => handleQuantityChange(item.id, item.quantity + 1)}
                      ...
                      <button
                        ...
                        disabled={isUpdating}
                        className="flex items-center justify-center gap-1 text-xs font-medium text-red-500 hover:text-red-600 hover:bg-red-50 px-3 py-1.5 rounded transition-colors w-full"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-lg p-4 sm:p-5 lg:p-6 lg:sticky lg:top-24">
                <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">Podsumowanie zamówienia</h2>
                  ...
                  {cart.couponCode ? (
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors shrink-0"
                      >
                        Zastosuj
                      </button>
                    </div>
                ...
                <div className="border-t mt-3 sm:mt-4 pt-3 sm:pt-4">
                  <div className="flex justify-between items-baseline">
                    <span className="text-base sm:text-lg font-semibold text-gray-900">Razem</span>
                    <div className="text-right">
                      <span className="text-xl sm:text-2xl font-bold text-gray-900">{cart.total.toFixed(2)} zł</span>
                      <p className="text-[10px] sm:text-xs text-gray-500">w tym VAT</p>
                    </div>
                  </div>
                </div>
                ...
                <Link
                  href="/checkout"
                  className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-2.5 sm:py-3 lg:py-3.5 rounded-lg mt-4 sm:mt-5 lg:mt-6 transition-colors flex items-center justify-center gap-2 text-sm sm:text-base"
                >
                ...
                <Link
                  href="/products"
                  className="w-full border border-gray-300 text-gray-700 hover:bg-gray-50 font-medium py-2.5 sm:py-3 lg:py-3.5 rounded-lg mt-2 sm:mt-3 transition-colors flex items-center justify-center gap-2 text-sm sm:text-base"
                >
                  <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="mt-4 sm:mt-5 lg:mt-6 pt-4 sm:pt-5 lg:pt-6 border-t">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-gray-500 mt-1.5 sm:mt-2">
                    <svg className="w-4 h-4 sm:w-5 sm:h-5 text-green-500 shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        {suggestedProducts.length > 0 && (
          <div className="mt-8 sm:mt-12 lg:mt-16 bg-white rounded-xl p-4 sm:p-6 lg:p-8">
            <div className="flex items-center justify-between mb-4 sm:mb-6 lg:mb-8">
              <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900">Może Cię zainteresować</h2>
              <Link href="/products" className="text-sm sm:text-base text-orange-500 hover:text-orange-600 font-medium flex items-center gap-1">
                Zobacz więcej
                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
              {suggestedProducts.slice(0, 5).map((product) => (
                <div
                  key={product.id}
                  className="bg-gray-50 rounded-xl p-3 sm:p-4 hover:shadow-lg transition-all hover:-translate-y-1 group"
                >
                  <Link href={`/products/${product.id}`}>
                    ...
                    <h3 className="text-sm sm:text-base font-medium text-gray-900 line-clamp-2 group-hover:text-orange-500 transition-colors mb-3 min-h-[40px] sm:min-h-[48px]">
                      {product.name}
                    </h3>
                  </Link>
                  ...
                  <button 
                    ...
                    className="w-full mt-3 sm:mt-4 bg-orange-500 hover:bg-orange-600 text-white text-xs sm:text-sm font-medium py-2 sm:py-2.5 rounded-lg transition-colors"
                  >
                    Do koszyka
                  </button>
                </div>
        ...
        <div className="mt-8 sm:mt-12 lg:mt-16 bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-4 sm:p-6 lg:p-10 text-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6 lg:gap-8">
            <div className="text-center">
              ...
              <h3 className="text-sm sm:text-base lg:text-lg font-bold mb-1 sm:mb-2">Najniższe ceny</h3>
              <p className="text-white/80 text-[11px] sm:text-xs lg:text-sm">Gwarancja najlepszej ceny na rynku.</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-white/20 rounded-2xl flex items-center justify-center mx-auto mb-2 sm:mb-3 lg:mb-4">
                <svg className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <h3 className="text-sm sm:text-base lg:text-lg font-bold mb-1 sm:mb-2">Błyskawiczna dostawa</h3>
              <p className="text-white/80 text-[11px] sm:text-xs lg:text-sm">Wysyłka w ciągu 24-72h</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-white/20 rounded-2xl flex items-center justify-center mx-auto mb-2 sm:mb-3 lg:mb-4">
                <svg className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <h3 className="text-sm sm:text-base lg:text-lg font-bold mb-1 sm:mb-2">Bezpieczne zakupy</h3>
              <p className="text-white/80 text-[11px] sm:text-xs lg:text-sm">Szyfrowane płatności SSL i pełna ochrona Twoich danych osobowych</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-white/20 rounded-2xl flex items-center justify-center mx-auto mb-2 sm:mb-3 lg:mb-4">
                <svg className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <h3 className="text-sm sm:text-base lg:text-lg font-bold mb-1 sm:mb-2">Łatwy zwrot</h3>
              <p className="text-white/80 text-[11px] sm:text-xs lg:text-sm">14 dni na bezpłatny zwrot towaru. Pełne wsparcie klienta.</p>
            </div>
          </div>
        </div>
      </main>
