...
export default function TransferPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      // Load variants with stock info
      const varRes = await api.get('/api/inventory/variants', token);
      if (varRes.ok) {
        const varData = await varRes.json();
        setVariants(varData.filter((v: Variant) => v.totalStock > 0));
      }
    } catch (error) {
  ...
  const addItem = (variant: Variant, location: VariantLocation) => {
    if (items.some(i => i.variantId === variant.id && i.fromLocationId === location.locationId)) {
      return;
    }
  ...
  const updateQuantity = (id: string, quantity: number) => {
    setItems(items.map(i => 
      i.id === id ? { ...i, quantity: Math.max(1, Math.min(quantity, i.maxAvailable)) } : i
    ));
  };

  const handleSubmit = async () => {
    if (!token || items.length === 0 || !toLocationId) {
      setError('Wybierz lokalizację docelową i dodaj produkty');
      return;
    }

    // Validate: can't transfer to same location
    for (const item of items) {
      if (item.fromLocationId === toLocationId) {
        setError('Nie można przenieść towaru do tej samej lokalizacji');
        return;
      }
    }
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (!res.ok) {
          ...
          throw new Error(data.message || 'Błąd przesunięcia towaru');
        }
      }
  ...
  // Get available target locations (exclude source locations)
  const sourceLocationIds = new Set(items.map(i => i.fromLocationId));
  const availableTargetLocations = locations.filter(l => !sourceLocationIds.has(l.id));
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Towar przesunięty!</h2>
          <p className="text-gray-400">Przeniesiono {items.length} pozycji ({totalItems} szt.)</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Przesunięcie międzymagazynowe</h1>
          <p className="text-gray-400">Przenieś towar między lokalizacjami</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-6">
            <h3 className="text-lg font-medium text-white mb-4">Informacje o przesunięciu</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Powód przesunięcia..."
                  rows={2}
          ...
          <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-white">Produkty do przeniesienia ({items.length})</h3>
              <button
              ...
              <div className="mb-4 p-4 bg-slate-700/50 rounded-lg border border-slate-600">
                ...
                <div className="max-h-80 overflow-y-auto space-y-2">
                  {filteredVariants.slice(0, 10).map(variant => (
                    ...
                    >
                      ...
                      <div className="pl-13 space-y-1">
                        {variant.locations.filter(l => l.available > 0).map(loc => (
                          <button
                            key={loc.locationId}
                            onClick={() => addItem(variant, loc)}
                            disabled={items.some(i => i.variantId === variant.id && i.fromLocationId === loc.locationId)}
                            className="w-full flex items-center justify-between p-2 bg-slate-600 rounded hover:bg-slate-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-left text-sm"
                          >
                  ...
                  {filteredVariants.length === 0 && (
                    <p className="text-center text-gray-400 py-4">Brak produktów z dostępnym stanem</p>
                  )}
                </div>
                <button
            ...
            {items.length === 0 ? (
              <div className="text-center py-12 text-gray-400">
                ...
                <p>Brak pozycji do przeniesienia</p>
                <p className="text-sm mt-2">Wybierz produkty i ich lokalizacje źródłowe</p>
              </div>
            ) : (
              <div className="space-y-3">
                {items.map((item, index) => {
                  const fromLocation = item.variant?.locations.find(l => l.locationId === item.fromLocationId);
                  return (
                    <div
                      key={item.id}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1 text-xs">
                          <span className="flex items-center gap-1 text-yellow-400">
                            ...
                            Z: {fromLocation?.locationName}
                          </span>
                          <ArrowRightLeft className="w-3 h-3 text-gray-500" />
                          <span className="flex items-center gap-1 text-green-400">
                            <MapPin className="w-3 h-3" />
                            Do: {locations.find(l => l.id === toLocationId)?.name || '...'}
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center bg-slate-600 text-white rounded hover:bg-slate-500 disabled:opacity-50"
                        >
                          +
                        </button>
